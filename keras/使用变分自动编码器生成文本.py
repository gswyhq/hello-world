...
# 我们将使用预训练的 Glove 词嵌入作为我们网络的嵌入。我们为词汇表中的每个单词创建一个嵌入的矩阵，然后将该矩阵作为权重传递给模型的 keras 嵌入层
embeddings_index = {}
...
# 现在我们构建了一个编码器模型模型，它接收一个句子并将其投影到潜在空间和一个从潜在空间返回到文本表示的解码器模型
...
sentence1 = ['我在哪里可以找到机器学习方面的书']
sentence2 = ['我怎样才能成为一名成功的企业家']
