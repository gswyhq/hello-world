...
class LngLatTransfer():
    '''
    常用的有WGS84、GCJ02（火星坐标系）、BD09（百度坐标系）以及百度地图中保存矢量信息的web墨卡托
    国际标准的经纬度坐标是WGS84，Open Street Map、外国版的Google Map都是采用WGS84；
    ...
    def GCJ02_to_BD09(self, gcj_lng, gcj_lat):
        """
        实现GCJ02（火星坐标系）向BD09（百度坐标系）坐标系的转换
    ...
    def BD09_to_GCJ02(self, bd_lng, bd_lat):
        '''
        实现BD09（百度坐标系）坐标系向GCJ02（火星坐标系）坐标系的转换
