...
>>> post_id
ObjectId('...')
如果文档中没有_id这个属性，那么当它插入集合中的时候，会自动给它赋予这样一个属性，同一个集合中每个文档的_id属性值必须具有唯一性。insert_one()返回一个InsertOneResutl实例。
...
MongoDB中最常用的基本操作是find_one(), 这个方法返回查询匹配到的第一个文档，如果没有则返回None。下面我们使用find_one()来获取posts集合中的第一个文档：

>>> posts.find_one()
{u'date': datetime.datetime(...), u'text': u'My first blog post!', u'_id': ObjectId('...'), u'author': u'Mike', u'tags': [u'mongodb', u'python', u'pymongo']}
查询结果是一个字典，内容正是我们之前插入的博客内容。

我们也可以使用find_one()来查找满足匹配要求的文档。例如，下面的例子查找作者名为"Mike"的文档。
...
>>>
在Web应用开发过程中，经常面临的一个任务就是从URL请求中获取ObjectId，然后根据这个ID查找匹配的文档。所以有必要在把ID传递给find_one()之前，对于字符串形式的id做一个转换，将其转换成ObjectID对象。
...
除了插入单个文档，我们还可以批量插入多个文档，通过给insert_many()方法传递一个列表，它会插入列表中的每个文档。
...
为了查询多个文档，我们可以使用find()方法，find()方法返回一个Cursor对象，使用这个对象可以遍历所有匹配的文档。例如下面的例子：
...
跟find_one()一样，我们也可以给find()方法设置匹配条件，例如，查询作者名为"Mike"的文档：
...
如果我们想获取匹配到文档个数，那么可以使用count()方法，使用这个方法还可以获取集合中文档的个数：
...
{u'date': datetime.datetime(2009, 11, 10, 10, 45), u'text': u'and pretty easy too!', u'_id': ObjectId('...'), u'author': u'Eliot', u'title': u'MongoDB is fun'}
{u'date': datetime.datetime(2009, 11, 12, 11, 14), u'text': u'Another post!', u'_id': ObjectId('...'), u'author': u'Mike', u'tags': [u'bulk', u'insert']}
这个我们使用了$lt操作符实现范围查询，还使用sort()方法对查询到的结果进行排序。

索引

添加索引可以加快查询的速度，在这个例子中，将展示如何创建一个唯一索引。 首先，创建索引:
