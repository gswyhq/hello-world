...
class Consumer():
    def __init__(self, topic=topic, group_id=group_id, bootstrap_servers=None, api_version=(1, 0, 0)):
        self.consumer = kafka.KafkaConsumer(topic, group_id=group_id, bootstrap_servers=bootstrap_servers, api_version=api_version,
    ...
    def test_poll(self, topic=None):
        ...
        # 若生产者仅有一个分区的话，那可能会出现只有一个消费者接收到消息，其他的消费者都接收不到消息
        # self.consumer.subscribe(topics=[topic])

        # partitions = self.consumer.partitions_for_topic(self.topic)
        # partition = TopicPartition(self.topic, 0)
        # self.consumer.assign(partitions=[partition])
        index = 0
        while True:
            msg = self.consumer.poll(timeout_ms=5, max_records=1)  # 从kafka获取max_records条消息
...
def main():
    con = Consumer(bootstrap_servers=bootstrap_servers)
    con.test_poll()
    # con.recv()
