    ...
    def __iter__(self):
        for fname in os.listdir(self.dirname):
            if self.key and(self.key not in fname):
                continue
            if fname.replace('.scel','.json') in os.listdir(self.dirname):
                continue
            yield os.path.join(self.dirname, fname)

def main ():
    if len(sys.argv)!=2 and len(sys.argv)!=3:
        print ("参数不对 \n 用法: python3 %s file.scel new.json" %(sys.argv[0]))
        exit (1)
    ...
    if os.path.isdir(sys.argv[1]):
        ...
        num=0
        path=sys.argv[1]
        files=[os.path.join(path,f)for f in os.listdir(path) if '.scel'in f ]
        for fileName in files:
            if len(sys.argv)!=3:
                outfile=fileName.replace('.scel','.json')
            else:
                outfile=sys.argv[2]
            try:
                generator = get_word_from_sogou_cell_dict(fileName)
                with open(outfile, "a",encoding='utf-8') as f:
                    json.dump(generator,f,ensure_ascii=0)
                logging.warning("\n{}解码完成！".format(fileName))
            except (struct.error,IndexError):
                num+=1
                logging.warning("\n{}解码失败，可能是下载不完整！".format(fileName))
        logging.warning("\n共有{}个文件解码失败！".format(num))

    else:
        try:
            generator = get_word_from_sogou_cell_dict (sys.argv[1])
            if len(sys.argv)!=3:
                outfile=sys.argv[1].replace('.scel','.json')
            else:
                outfile=sys.argv[2]
