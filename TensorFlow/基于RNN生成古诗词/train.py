...
class ArgsError(ValueError):
    def __init__(self, model):
        print("模型参数错误,不应该为： {}".format(model))

# 定义RNN
def neural_network(model='lstm', rnn_size=128, num_layers=2, batch_size=64, input_data=None):
    if model == 'rnn':
...
def train_neural_network():
    logits, last_state, _, _, _ = neural_network(batch_size=batch_size)
